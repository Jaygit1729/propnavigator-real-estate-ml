...
def column_cleaning(df):
    """
    Cleans and transforms specific columns in the Indepedent House dataset:
    ...
    try:
        ...
        logger.info("Independent House data column cleaning done.")
        return df
    except Exception as e:
        logger.error(f"Error during house column cleaning: {e}")
        return None


def clean_house_data(file_path):
    """
    Runs the full data cleaning pipeline for house data:
    - Load CSV
    ...
    try:
        ...
        if df is not None:
            logger.info(f"Independent House data cleaned and shape is {df.shape}.")
        return df
    except Exception as e:
        logger.error(f"Error during house data cleaning for file {file_path}: {e}")
        return None
